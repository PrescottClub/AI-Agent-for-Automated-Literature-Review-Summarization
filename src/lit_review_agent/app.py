...
def inject_custom_css():
    ...
    # .parent.joinpath("assets", "styles.css") 将指向
    # src/lit_review_agent/assets/styles.css
    ...
    if custom_css:
        ...
        st.info(
            "Attempted to load custom styles, but the CSS file was "
            "empty or not found."
        )
...
def display_metrics(results: Dict):
    ...
    if results and "processed_papers" in results:
        ...
        with col2:
            full_text_count = sum(
                1 for p in papers if p.get("full_text_retrieved", False)
            )
            st.markdown(
                f"""
            <div class="metric-card-modern">
...
def display_paper_card(paper: Dict, index: int):
    ...
    with st.container():
        # 开始卡片容器
        st.markdown(
            '<div class="paper-card-modern fade-in-up">', unsafe_allow_html=True
        )

        # 论文编号（右上角）
        st.markdown(
            f'<div class="paper-number-modern">#{index}</div>',
            unsafe_allow_html=True
        )

        # 论文标题
        st.markdown(
            f'<div class="paper-title-modern">'
            f'{paper.get("title", "未知标题")}</div>',
            unsafe_allow_html=True,
        )
        ...
        with col3:
            source_name = paper.get('source', 'unknown').upper()
            st.markdown(f"📊 **来源:** {source_name}")

        # 摘要
        summary = paper.get(
            "ai_enhanced_summary", paper.get("original_summary", "暂无摘要")
        )
        if len(summary) > 300:
            summary = summary[:300] + "..."
        ...
        if keywords:
            ...
            for i, kw in enumerate(keywords[:6]):
                with keyword_cols[i]:
                    st.markdown(
                        f'<span class="keyword-tag-modern">{kw}</span>',
                        unsafe_allow_html=True,
                    )
        ...
        if paper.get("url") or paper.get("pdf_url"):
            ...
            if paper.get("url"):
                with btn_col1:
                    st.markdown(
                        ...
                        f'class="paper-link-modern">📄 查看原文</a>',
                        unsafe_allow_html=True,
                    )
            ...
            if paper.get("pdf_url"):
                with btn_col2:
                    st.markdown(
                        f'<a href="{paper["pdf_url"]}" target="_blank" '
                        f'class="paper-link-modern">📁 PDF下载</a>',
                        unsafe_allow_html=True,
                    )
...
def main():
    ...
    with st.sidebar:
        with st.container():
            ...
            st.markdown(
                '<div class="sidebar-title">🎛️ 检索配置</div>',
                unsafe_allow_html=True
            )
            ...
            with col1:
                year_start = st.number_input(
                    "起始年份", min_value=1990, max_value=2024, value=2020, step=1
                )
            with col2:
                year_end = st.number_input(
                    "结束年份", min_value=1990, max_value=2024, value=2024, step=1
                )
    ...
    with main_container:
        ...
        if st.session_state.get("results"):
            ...
            papers_count = results.get('num_papers_processed', 0)
            st.success(f"✅ 成功检索到 {papers_count} 篇相关文献")
            ...
            if results.get("processed_papers"):
                ...
                if sort_by == "按时间降序":
                    ...
                    papers = sorted(
                        papers, key=lambda x: x.get("title", "").lower()
                    )
