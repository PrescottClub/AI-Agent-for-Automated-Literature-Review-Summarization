...
class VectorStore(LoggerMixin):
    ...
    def __init__(self,
        ...
        try:
            ...
            self.logger.info(f"Initialized ChromaDB client at {persist_directory}")
    ...
    def add_literature_item(self, item: LiteratureItem) -> bool:
        ...
        try:
            ...
            if not text_content or not self.embedding_model:
                self.logger.warning(f"Cannot create embedding for item {item.id}")
                return False
            ...
            self.logger.info(f"Added literature item to vector store: {item.id}")
            return True
            
        except Exception as e:
            self.logger.error(f"Error adding item {item.id} to vector store: {e}")
            return False
        ...
        for i in range(0, len(items), batch_size):
            ...
            if batch_ids:
                try:
                    ...
                    self.logger.info(f"Added batch of {len(batch_ids)} items to vector store")
                except Exception as e:
                    self.logger.error(f"Error adding batch to vector store: {e}")
        
        self.logger.info(f"Successfully added {successful_additions}/{len(items)} items")
    ...
    def search_similar(self,
        ...
        try:
            ...
            self.logger.info(f"Found {len(formatted_results)} similar items for query")
    ...
    def _prepare_text_for_embedding(self, item: LiteratureItem) -> str:
        ...
        if item.full_text:
            # Take first 1000 characters of full text
            truncated_text = item.full_text[:1000]
