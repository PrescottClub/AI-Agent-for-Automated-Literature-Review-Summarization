...
class TextProcessor(LoggerMixin):
    ...
    def __init__(self, 
        ...
        try:
            ...
            self.logger.warning(f"Could not load spaCy model {spacy_model}, using basic processing")
            self.nlp = None
    ...
    def extract_research_keywords(self, text: str, max_keywords: int = 20) -> List[str]:
        ...
        research_indicators = {
            'algorithm', 'model', 'method', 'approach', 'framework', 'system',
            'analysis', 'evaluation', 'experiment', 'study', 'research',
            'technique', 'strategy', 'mechanism', 'process', 'procedure',
            'optimization', 'performance', 'accuracy', 'efficiency',
