...
class TrendAnalyzer(LoggerMixin):
    ...
    async def analyze_temporal_trends(self, 
                                    papers: List[LiteratureItem]) -> Dict[str, Any]:
        """
        ...
        for paper in papers:
            if paper.publication_date:
                ...
                yearly_data[year]['keywords'].extend(paper.keywords or [])
                yearly_data[year]['categories'].extend(paper.categories or [])
        ...
        trend_analysis = {
            ...
            'yearly_summary': {year: data['count'] for year, data in yearly_data.items()}
        }
        ...
        if len(yearly_data) > 1:
            ...
            for year, data in sorted(yearly_data.items()):
                ...
                if data['keywords']:
                    ...
                    year_text += f"Top keywords: {', '.join([kw for kw, _ in top_keywords])}. "
    ...
    async def analyze_keyword_trends(self, 
                                   papers: List[LiteratureItem]) -> Dict[str, Any]:
        """
        ...
        trend_analysis = {
            ...
            'keyword_cooccurrence': sorted(keyword_cooccurrence.items(), 
    ...
    async def analyze_collaboration_patterns(self, 
        ...
        for paper in papers:
            if paper.authors:
                paper_authors = [author.strip() for author in paper.authors]
                ...
                for i, author1 in enumerate(paper_authors):
                    for author2 in paper_authors[i+1:]:
                        ...
                        author_collaboration[pair] = author_collaboration.get(pair, 0) + 1
        
        # Analyze author productivity
        author_freq = Counter(all_authors)
        
        # Analyze team sizes
        team_sizes = [len(paper.authors) for paper in papers if paper.authors]
        avg_team_size = sum(team_sizes) / len(team_sizes) if team_sizes else 0
        
        collaboration_analysis = {
            'most_productive_authors': author_freq.most_common(15),
            'frequent_collaborations': sorted(author_collaboration.items(), 
    ...
    async def analyze_methodological_trends(self, 
                                          papers: List[LiteratureItem]) -> Dict[str, Any]:
        """
        ...
        for paper in papers:
            if paper.abstract:
                ...
                texts.append(paper.full_text[:1000])
    ...
    async def identify_emerging_topics(self, 
                                     papers: List[LiteratureItem],
                                     min_year: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        recent_papers = [
            ...
            if paper.publication_date and paper.publication_date.year >= min_year
        ]
        ...
        for paper in recent_papers:
            if paper.abstract:
                recent_texts.append(paper.abstract)
        
        # Use AI to identify emerging topics
        emerging_analysis = await self._identify_emerging_topics_ai(recent_texts, min_year)
    ...
    def _analyze_publication_growth(self, yearly_data: Dict[int, Dict]) -> Dict[str, Any]:
        ...
        return {
            ...
            'total_growth': round(((counts[-1] - counts[0]) / counts[0]) * 100, 2) if counts[0] > 0 else 0
        }
    ...
    def _analyze_evolving_categories(self, yearly_data: Dict[int, Dict]) -> Dict[str, List]:
    ...
    def _analyze_keyword_by_year(self, papers: List[LiteratureItem]) -> Dict[str, Dict]:
        ...
        for paper in papers:
            if paper.keywords and paper.publication_date:
        ...
        return {year: dict(counter.most_common(10)) for year, counter in keyword_by_year.items()}
    ...
    def _extract_methodology_keywords(self, texts: List[str]) -> Dict[str, int]:
        ...
        methodology_terms = [
            ...
            'survey', 'experiment', 'case study', 'simulation', 'modeling',
            'dataset', 'benchmark', 'evaluation', 'validation', 'cross-validation',
    ...
    def _analyze_keyword_emergence(self, papers: List[LiteratureItem], min_year: int) -> List[str]:
        ...
        for paper in papers:
            if paper.keywords and paper.publication_date:
                year = paper.publication_date.year
                keywords = [kw.lower().strip() for kw in paper.keywords]
    ...
    async def _generate_temporal_analysis(self, yearly_texts: List[str]) -> Optional[str]:
        ...
        user_prompt = (
            f"Analyze these yearly publication trends and identify key patterns:\n\n{combined_text}"
        )
    ...
    async def _identify_emerging_topics_ai(self, texts: List[str], min_year: int) -> Optional[str]:
