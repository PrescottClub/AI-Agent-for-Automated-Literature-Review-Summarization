...
class LiteratureReviewDisplay:
    ...
    def print_status(self, message: str, style: str = "bold green") -> None:
        ...
        self.console.print(f"[dim]{timestamp}[/dim] [bold blue]→[/bold blue] [{style}]{message}[/{style}]")
    ...
    def update_progress(self, advance: int = 1, description: Optional[str] = None) -> None:
        ...
        if self.current_progress and self.current_task is not None:
            if description:
                self.current_progress.update(self.current_task, description=description, advance=advance)
            else:
        ...
        for i, paper in enumerate(papers[:20], 1):  # Limit to first 20 for display
            ...
            if pub_date and pub_date != 'N/A':
                try:
                    ...
                    date_display = date_part.split('-')[0] if '-' in date_part else date_part
                except:
                    date_display = "N/A"
            else:
                date_display = "N/A"
    ...
    def create_summary_panel(self, results: Dict[str, Any]) -> Panel:
        ...
        if 'processed_papers' in results:
            ...
            summary_lines.append(f"📄 [bold]Full Text Retrieved:[/bold] {full_text_count}/{num_papers} papers")
