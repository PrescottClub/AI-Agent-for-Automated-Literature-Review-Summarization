...
class SemanticScholarClient(BaseRetriever, LoggerMixin):
    ...
    def __init__(
        ...
        self.api_key = (
            api_key  # May be needed for higher rate limits or specific features
        )
        ...
        self.logger.info(
            f"SemanticScholarClient initialized. API URL: {self.api_url}, API Key set: {bool(self.api_key)}"
        )
    ...
    def _parse_publication_date(
        self, pub_date_str: Optional[str]
    ) -> Optional[datetime]:
        if not pub_date_str:
            return None
        try:
    ...
    def _to_literature_item(self, paper_data: Dict[str, Any]) -> LiteratureItem:
        ...
        journal_name = (
            journal_info.get("name") if journal_info else paper_data.get("venue")
        )
    ...
    async def search(
        ...
        **kwargs,
        ...
        return await self.search_papers(
            query, max_results, fields, year_start, year_end
        )
    ...
    async def search_papers(
        ...
        if year_start and year_end and year_start > year_end:
            self.logger.warning(
                "year_start cannot be after year_end. Ignoring year filters."
            )
        ...
        self.logger.info(
            f"Searching Semantic Scholar for: '{query}' with limit {max_results}"
        )
        ...
        try:
            timeout_config = httpx.Timeout(
                self.config.pdf_processing_timeout if self.config else 30.0, read=60.0
            )
            async with httpx.AsyncClient(timeout=timeout_config) as client:
                ...
                if not papers_list and isinstance(
                    results_data, list
                ):  # If results_data itself is the list
                    papers_list = results_data
                elif (
                    not papers_list and "paperId" in results_data
                ):  # Single result if total is 1
                    papers_list = [results_data]

                if not papers_list and results_data.get("total", 0) > 0:
                    self.logger.warning(
                        f"Semantic Scholar returned total > 0 but no 'data' list. Response: {results_data}"
                    )

                self.logger.info(
                    f"Received {len(papers_list)} raw results from Semantic Scholar. Total reported: {results_data.get('total', 0)}"
                )
                ...
                self.logger.info(
                    f"Processed {len(items)} items from Semantic Scholar after filtering."
                )

        except httpx.HTTPStatusError as e:
            self.logger.error(
                f"HTTP error from Semantic Scholar API: {e.response.status_code} - {e.response.text}",
                exc_info=True,
            )
            raise SemanticScholarError(
                f"Semantic Scholar API request failed: {e.response.status_code}"
            ) from e
        except httpx.RequestError as e:
            self.logger.error(
                f"Request error connecting to Semantic Scholar API: {e}", exc_info=True
            )
            raise SemanticScholarError(
                f"Could not connect to Semantic Scholar API: {e}"
            ) from e
        except Exception as e:
            self.logger.error(
                f"Unexpected error during Semantic Scholar search: {e}", exc_info=True
            )
    ...
    async def get_by_id(self, item_id: str) -> Optional[LiteratureItem]:
        ...
        if s2_lookup_id.isdigit() and not any(
            s2_lookup_id.startswith(p) for p in ["DOI:", "ARXIV:", "CorpusId:"]
        ):
            s2_lookup_id = f"CorpusId:{s2_lookup_id}"
        elif "doi:" in s2_lookup_id.lower():
            s2_lookup_id = s2_lookup_id.upper().replace(
                "DOI:", "DOI:"
            )  # Ensure DOI prefix is correct case for S2
        ...
        self.logger.info(
            f"Fetching paper details from Semantic Scholar for ID: {s2_lookup_id}"
        )
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=timeout_config) as client:
                ...
                if paper_data and paper_data.get(
                    "paperId"
                ):  # Check if valid paper data returned
                    return self._to_literature_item(paper_data)
                elif paper_data.get(
                    "error"
                ):  # S2 sometimes returns 200 OK with an error message
                    self.logger.error(
                        f"Semantic Scholar API returned an error for ID {s2_lookup_id}: {paper_data['error']}"
                    )
                    return None
                else:  # No paperId and no explicit error, but not valid data
                    self.logger.warning(
                        f"No valid paper data found for ID {s2_lookup_id} from Semantic Scholar. Response: {paper_data}"
                    )
                    return None
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 404:
                self.logger.info(
                    f"Paper with ID {s2_lookup_id} not found on Semantic Scholar."
                )
                return None
            self.logger.error(
                f"HTTP error fetching details from S2 API: {e.response.status_code} - {e.response.text}",
                exc_info=True,
            )
            raise SemanticScholarError(
                f"S2 API request failed for paper details: {e.response.status_code}"
            ) from e
        except Exception as e:
            self.logger.error(
                f"Unexpected error fetching paper details from S2: {e}", exc_info=True
            )
            raise SemanticScholarError(
                f"An unexpected error occurred while fetching paper details: {e}"
            ) from e
...
async def main_s2_test():
    ...
    try:
        papers = await s2_client.search_papers(
            query, max_results=5, year_start=2020, year_end=2022
        )
        if papers:
            ...
            for i, paper in enumerate(papers):
                ...
                print(
                    f"     Abstract snippet: {paper.abstract[:100] if paper.abstract else 'N/A'}..."
                )
