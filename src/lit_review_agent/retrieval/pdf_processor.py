...
class PDFProcessor(LoggerMixin):
    ...
    async def extract_text_from_file(self, file_path: str) -> Optional[str]:
        ...
        try:
            ...
            # Try pdfminer first (usually more accurate)
            try:
                text = extract_text(file_path)
                if text and text.strip():
                    cleaned_text = clean_text(text)
                    self.logger.info(f"Extracted {len(cleaned_text)} characters using pdfminer")
                    return cleaned_text
            except Exception as e:
            ...
            try:
                with open(file_path, 'rb') as file:
                    ...
                    if text.strip():
                        cleaned_text = clean_text(text)
                        self.logger.info(f"Extracted {len(cleaned_text)} characters using PyPDF2")
                        return cleaned_text
